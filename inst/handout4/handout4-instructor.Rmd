...
set that encompasses over 70 countries and 200 years worth of observations 
...
This argument is not strictly academic; the current US debt-to-GDP ratio has almost doubled since the financial crisis and currently stands at about 105%. In Japan, the debt-to-GDP ratio is over 200%. If Reinhart and Rogoff's conclusions are correct then these countries could increase their GDP growth rates by cutting spending and/or raising taxes. This policy is referred to as "austerity". Policy makers argue over the potential benefits of austerity versus the possible harm that could come from cutting spending for government programs. US House Speaker Paul Ryan has even cited Reinhart and Rogoff when making arguments for cutting spending.
...
- Herndon et al.'s reply: @Herndon.etal2014; you should focus on the first 10 
  pages or so
- Stephen Colbert 
  [weighs in](http://www.cc.com/video-clips/dcyvro/the-colbert-report-austerity-s-spreadsheet-error) on the issue
...
- An early critique by Irons and Bivens: [@Irons.Bivens2010]
  ...
  on causality 
- A [blog post](http://rooseveltinstitute.org/researchers-finally-replicated-reinhart-rogoff-and-there-are-serious-problems/) on 'Rortybomb' laying out the issues involved

## Correlations and Linear Models

You will find the file `RR.handout.csv` in the data folder.
The file contains a subset of the variables considered by Herndon et al.:
...
These data are an example of **panel data**, a type of data where the same 
...
```
which gives  a correlation of approximately -0.19.  So, current debt-to-GDP ratios and current 
growth show a negative relationship.  We could also look at the correlation 
between past and current debt-to-GDP ratios:
```{r }
...
```
It appears that the debt-to-GDP ratio is highly *autocorrelated*. In other words, its past and current values correlate at a really high level.  The problem is less acute (though 
...
```
The first difference of debt ratio (`RR$debtgdp - RR$lag.debtgdp`) correlates with 
previous debt ratio at about $-0.24$, which is much less (in magnitude) than the original correlation of approximately $0.99$.  
...
a one percentage point increase in debt ratio on GDP growth is captured by the estimated 
...
plot(RR$debtgdp, RR$dRGDP, 
     ...
     main = "Debt-to-GDP Ratio versus Growth")
```
...
plot(RR$debtgdp, RR$dRGDP, 
     xlab = "Debt to GDP", ylab = "Change in GDP, %", 
     main = "Debt-to-GDP Ratio versus Growth", col = "red")
abline(lm1)
```
...
plot(RR$debtgdp, RR$dRGDP, 
     xlab = "Debt to GDP", ylab = "Change in GDP, %",
     main = "Debt-to-GDP Ratio versus Growth", col = col.plot)
abline(lm1)
legend(c("topright"), c("Increasing Debt Ratio", "Decreasing Debt Ratio"), 
...
We can also set `pch` to a letter: "A", "f", etc. This will use the letter A or f as the point character.
...
plot(RR$debtgdp, RR$dRGDP, 
     xlab = "Debt to GDP", ylab = "Change in GDP, %", 
     main = "Debt-to-GDP Ratio versus Growth", pch = pch.plot)
...
- *Current* debt ratio versus *current growth* for countries with increasing 
  debt ratios and *high* previous growth 
- *Current*  debt ratio versus *current growth* for countries with increasing 
  debt ratios and *low* previous growth
...
the previous to current year of 2 or more percentage points and  *high* and *low* previous debt as previous debt-to-GDP ratios above or below 70.  
...
```{r, fig.height = 5, fig.width = 9, solution = TRUE}
...
in the example (in the secton above)? In the left and right hand figures 
from Question 1?
...
Is the relationship between debt ratios and GDP more sensitive to changes in 
previous growth rates (in the section above) of previous shifts in the 
debt ratio (Question 1)?  
