...
- `X`. A vector of values to which we want to apply a function
- `INDEX`. A vector of group names within which we want to apply the function 
...
If we wanted the mean duration to conflict for each continent, for countries that 
...
The function `boxplot` can be used to construct separate boxes for each category 
...
Finally, we are going to add a command that allows for multiple plots in one figure.  To do so, we need to adjust R's default plot parameters.  We'll use the `par` function to
...
of peace (looking at the leftmost figure).  We see that the association is more significant 
...
Now calculate first the mean of 
`relief` by year and then the mean of `diff.vote` by year.  
```{r, solution = TRUE}
