...
In this handout, we will learn more about using the function `lm` and how to use residual and conditional plots.

## Topics and Concepts Covered

- `I()` to add new terms to a linear model
- `data` and `subset` options for `lm`
...
In addition to squaring values, `I` can be used for other operators 
such as `+`, `-`, `\`, and `*`.

# Using the `data` and `subset` options for the `lm` command

So far, you have had to use the syntax `data.frame$variable` in order to 
reference `variable` from the object `data.frame` when running a linear 
model with the `lm` command. `lm` includes an option, `data`, that allows 
you to specify what dataset you are using and then you can call the variable 
directly. The syntax for `lm` is
```{r, eval = FALSE}
lm(variable1 ~ variable2, data = data.frame)
```
As you have already seen in this course, there are many times when we want 
to only look at a subset of the data. Within the `lm` command we can subset 
the data using the `subset` option. The syntax for `subset` is
```{r, eval = FALSE}
lm(variable1 ~ variable2, data = data.frame, subset = rows)
```
You can also use the `data` and `subset` options with the `plot` command.
...
In a classic paper @Mankiw.etal1992 investigate the determinants of long-term 
economic growth. In particular, they empirically explore the Solow growth 
model using cross-country data. The paper uses as its starting point the 
Solow growth model, which predicts both the steady-state 
...
```{r }
Data.Growth <- read.csv("data/international_growth_data.csv")
names(Data.Growth)
```
The Solow growth model makes some very specific predictions about the 
relationships among these variables.  Specifically,
\begin{align*}
\log[\text{gdpperadult1985}_i] = & \alpha + \beta_k \log[\text{investmentpergdp}_i/100]\\  
& \quad + \beta_P \log[\text{workingagepopgrowth}_i/100 + 0.05]  + \epsilon_i
\end{align*}
The model predicts $\beta_k = \frac{1}{2}$, $\beta_p = -\frac{1}{2}$. 
Unfortunately, our data is defined in terms of the raw values, not logs. 
Therefore, in this example, we will need to log these variables. We can do 
this directly within the `lm` command.

```{r }
lm1 <- lm(log(gdp_per_adult_1985) ~ log(investment_per_gdp / 100) +
          log(working_age_pop_growth_1960to198 / 100 + .05), data = Data.Growth)

summary(lm1)
```
These names are too long, so let's clean them up through creating new 
variables, instead of using `log` within the `lm` command:
```{r }
log.dv <- log(Data.Growth$gdp_per_adult_1985)
log.invest <- log(Data.Growth$investment_per_gdp / 100)
log.pop <- log(Data.Growth$working_age_pop_growth_1960to198 / 100 + .05)
lm1 <- lm(log.dv ~ log.invest + log.pop, data = Data.Growth)
```
...
```{r}
pch.oecd <- ifelse(Data.Growth$oecd, 19, 1)
plot(lm1$fit, lm1$res, 
     main = "Fitted vs. Residual Plot",
     xlab = "Fitted Values", ylab = "Residuals", 
     pch = pch.oecd) 
legend(x = 6, y = 3,
       legend = c("OECD", "Non-OECD"),
       pch = c(19,1))
```
...
simply run
```{r }
pch.oecd <- ifelse(Data.Growth$oecd, "O", "X")
plot(lm1$fit, lm1$res, 
     main = "Fitted vs. Residual Plot", 
     xlab = "Fitted Values", ylab = "Residuals", 
     pch = pch.oecd) 
...
schooling:
```{r}
pch.schooling <- ifelse(Data.Growth$schooling > 8, 19, 1)
col.schooling <- ifelse(Data.Growth$schooling > 8, "red", "blue")
plot(lm1$fit, lm1$res, 
     main = "Fitted vs. Residual Plot", 
     xlab = "Fitted Values", ylab = "Residuals", 
     pch = pch.schooling, col = col.schooling) 
abline(h = mean(lm1$res[Data.Growth$schooling > 8], na.rm = TRUE), col = "red")
abline(h = mean(lm1$res[Data.Growth$schooling <= 8], na.rm = TRUE), lty = 2, col = "blue")
legend(x = 6, y = 3,
       legend = c("High Schooling", "Low Schooling"),
       pch = c(19,1),
       col = c("red","blue"))
```
...
```{r }
Data.Growth$log.schooling <- log(Data.Growth$schooling)
lm2 <- lm(log.dv ~ log.invest + log.pop + log.schooling, data = Data.Growth)
summary(lm2)
```
...
data from Handout 2. The file `RossOilWomenIslamData.csv` can be found in the 
`data` folder.
...
'high Muslim' countries, for which `islam > 0` with solid dots, and 'low Muslim' 
...
The figure should show a clear 
interaction between proportion Muslim, oil, and Middle Eastern countries. 
Where do we see a relationship between proportion Muslim and 
female labor participation and where not?
