...
```{r}
Data.SAF <- read.csv("data/subset_saf.csv")
head(Data.SAF)
```
...
Now, 999 is commonly used to denote missing data.  If 999 were a marker for 
missing  data here, we would expect to see a top age of somewhere in the 90's, 
and then a big gap between the highest age and 999.  That is not the case 
here.  But we do see people recorded at 230, 245, 347, and 396 years old.
...
informative using information from the code book, which can be found 
[here](http://michael.hahsler.net/research/arules_RUG_2015/demo/SQF_Codebook.pdf).
```{r}
...
Here's the first three columns:
```{r}
...
Note: Saving a figure in RStudio can be accomplished through clicking on "Export" on
