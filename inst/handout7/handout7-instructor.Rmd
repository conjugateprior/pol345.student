...
look at it:
```{r }
lm1 <- lm(mistake ~ female, data = iraq)
lm1
...
lm2 <- lm(log.growth ~ war, data = growth) # Estimate diff in means
summary(lm2) 
```
...
Load in the data.  Was the treatment effect positive or negative? Was it statistically significant?  
...
called `GerberGreenSubset.txt`, which contains the following variables:
 ...
 `mailing`               Number of mailings sent (0,1,2,3)
 ----------------------------------------------------------------------------

You notice that mailings have no significant effect on turnout. But pretend 
